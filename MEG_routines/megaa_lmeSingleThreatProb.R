...
# remove NaNs
indx <- which(is.nan(GO) | TL == 2 | TL == 3)
SUBJ <- SUBJ[-indx]
RUNN <- RUNN[-indx]
TRLL <- TRLL[-indx]
TL   <- TL[-indx]
...
F.Real.Cau <- F.Real.Col <- matrix(NaN, ncol = MinDur, nrow = 3)
...
for (t in 1:MinDur){
  ...
  # CAU
  # -----------------------------------------------------------
  mdl.Real.Cau[[t]] <- lmer(Y.Cau ~ 1 + PL*GO + (1|SUBJ), data = data.Cau)
  F.Real.Cau[,t] <- anova(mdl.Real.Cau[[t]])[ ,4]
  
  # COL
  # -----------------------------------------------------------
  mdl.Real.Col[[t]] <- lmer(Y.Col ~ 1 + PL*GO + (1|SUBJ), data = data.Col)
  F.Real.Col[,t] <- anova(mdl.Real.Col[[t]])[ ,4]
  
}

# analyse permutations
# -------------------------------------------------------------
F.Perm.Cau <- F.Perm.Col <- array(NaN, dim = c(3,MinDur,NumPerm))
if (permute == 1){
  for (p in 1:NumPerm){
    ...
    for (t in 1:MinDur){
      ...
      mdl.Perm.Cau  <- lmer(Y.Perm.Cau ~ 1 + PL*GO + (1|SUBJ), data = data.Perm.Cau)
      mdl.Perm.Col  <- lmer(Y.Perm.Col ~ 1 + PL*GO + (1|SUBJ), data = data.Perm.Col)
      
      # Results from the ANOVA
      F.Perm.Cau[,t,p] <- anova(mdl.Perm.Cau)[ ,4]
      F.Perm.Col[,t,p] <- anova(mdl.Perm.Col)[ ,4]
