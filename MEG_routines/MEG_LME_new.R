...
F.Real.Cau <- F.Real.Col <- matrix(NaN, ncol = MinDur, nrow = 4)
...
for (t in 1:MinDur){
  ...
  # CAU
  # -----------------------------------------------------------
  mdl.Real.Cau[[t]] <- lmer(Y.Cau ~ 1 + GO*PREV + (1|SUBJ), data = data.Cau)
  F.Real.Cau[1:4,t] <- anova(mdl.Real.Cau[[t]])[1:4,4]
  
  # COL
  # -----------------------------------------------------------
  mdl.Real.Col[[t]] <- lmer(Y.Col ~ 1 + TL*PL*GO*PREV + (1|SUBJ), data = data.Col)
  F.Real.Col[1:4,t] <- anova(mdl.Real.Col[[t]])[1:4,4]
...
title(main="PREV - Cau", col.main="black", font.main=3, cex.main = 1.5)
plot(1:MinDur,F.Real.Col[4, ], col="blue")
title(main="PREV - Col", col.main="black", font.main=3, cex.main = 1.5)
